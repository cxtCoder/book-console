    ...
    <div>
            ...
            <el-row>
                ...
                <el-col :span="5" v-if="showMore">
                    <el-form-item label="规格">
                        <el-input v-model="selectForm.bookSubName" placeholder="规格"></el-input>
                    </el-form-item>
                </el-col>
                <el-col :span="5" v-if="showMore">
                    <el-form-item label="材质">
                        <el-input v-model="selectForm.publishing" placeholder="材质"></el-input>
                    </el-form-item>
                </el-col>
                <el-col :span="5" v-if="showMore">
        ...
        <div class="book-summary-single">
            <div style="font-size: 15px; margin-top: 3px">
                <label>{{ classify }}总数：{{ totalNumberForClassify }}幅</label>
            </div>
        </div>
        <div v-if="showList">
            <el-table :data="bookDatas" stripe border tooltip-effect="dark" v-loading="loading" element-loading-text="拼命加载中" @selection-change="selectionChange">
                ...
                <el-table-column prop="thumbPath" align="center" label="书画图">
                ...
                <el-table-column prop="quantity" align="center" label="数量(幅)"></el-table-column>
                        ...
                        <div style="padding: 8px 0; border-bottom: 1px #ddd solid">
                            <el-row>
                                ...
                                <el-col :span="19">
                                    ...
                                    <div>
                                        <span style="color: rgb(104, 171, 248)">创作日期：</span><span>{{ data.publishedDate }}</span>
                                        <span style="margin-left: 50px"></span>
                                        <span style="color: rgb(104, 171, 248)">规格：</span>
                                        <span v-if="data.bookSubName.length > 45">{{ data.bookSubName.substring(0, 45) }} ...</span>
        ...
        methods: {
                    ...
                    .then(function (result) {
                        ...
                        if (response.suc) {
                            ...
                            if (_this.bookAllData.length == 0) {
                                _this.$message.warning("暂无名人书画数据");
                                return;
                            }
                            _this.export2Excel();
                        } else {
            ...
            deleteBooks() {
                ...
                if (_this.selectedIds.length == 0) {
                    _this.$message.warning("请选择需要删除的名人书画");
                    return;
                }
                _this.$confirm(`确定删除该名人书画数据？`, '提示', {
            ...
            addBook() {
                this.$emit("update:showPage", "paintingManage");
            ...
            details(row) {
                this.$emit("update:showPage", "paintingDetail");
            ...
            export2Excel() {
                require.ensure([], () => {
                    const tHeader = ['省份', '城市', '区/县', '一级分类', '二级分类', '名称', '作者', '规格', '材质', '数量',
                                       '创作日期', '简介', '存放位置', '备注', '录入者', '录入时间', '更新时间'];
                    const filterVal = ['province', 'city', 'district', 'classify', 'subClassify', 'bookName', 'author', 'bookSubName', 'publishing', 'quantity',
                                        'publishedDate', 'summary', 'position', 'remarks', 'createUser', 'createTime', 'updateTime'];
