    ...
    <div>
            ...
            <el-row>
                ...
                <el-col :span="5" v-if="showMore">
                    <el-form-item label="级别">
                        <el-input v-model="selectForm.author" placeholder="级别"></el-input>
                    </el-form-item>
                </el-col>
            ...
            <el-row>
                ...
                <el-col :span="5" v-if="showMore">
                    <el-form-item label="名称">
                        <el-input v-model="selectForm.bookName" placeholder="名称"></el-input>
                    </el-form-item>
                </el-col>
                <el-col :span="5" v-if="showMore">
                    <el-form-item label="年代">
                        <el-input v-model="selectForm.bookSubName" placeholder="所属年代"></el-input>
                    </el-form-item>
                </el-col>
                <el-col :span="5" v-if="showMore">
                    <el-form-item label="地点">
                        <el-input v-model="selectForm.publishing" placeholder="出土地点"></el-input>
                    </el-form-item>
                </el-col>
                <el-col :span="5" v-if="showMore">
        ...
        <div style="padding: 10px 0">
            <el-row class="book">
                ...
                <el-col v-if="userType === '0'" :span="22">
                    <el-button type="primary" class="glyphicon glyphicon-export" @click="outExcel">导出</el-button>
                </el-col>
                <el-col :span="colSize">
        ...
        <div class="book-summary-single">
            <div style="font-size: 15px; margin-top: 3px">
                <label>{{ classify }}总数：{{ totalNumberForClassify }}件</label>
            </div>
        </div>
        <div v-if="showList">
            <el-table :data="bookDatas" stripe border tooltip-effect="dark" v-loading="loading" element-loading-text="拼命加载中" @selection-change="selectionChange">
                ...
                <el-table-column prop="thumbPath" align="center" label="文物图">
                ...
                <el-table-column prop="bookName" align="center" label="名称" show-overflow-tooltip></el-table-column>
                <el-table-column prop="bookSubName" align="center" label="年代"></el-table-column>
                <el-table-column prop="quantity" align="center" label="数量(件)"></el-table-column>
                    ...
                    <li style="list-style-type:none;" v-on:mouseenter="onMouseOver(index)" v-on:mouseleave="onMouseOut(index)">
                        <div style="padding: 8px 0; border-bottom: 1px #ddd solid">
                            <el-row>
                                ...
                                <el-col :span="19">
                                    <div><label style="font-size: 18px; padding-bottom: 10px">{{ data.bookName }}</label></div>
                                    <div>
                                        <span style="color: rgb(104, 171, 248)">地区：</span>
                                        <span>{{ data.province }} / {{ data.city }} / {{ data.district }}</span>
                                    </div>
                                    <div>
                                        <span style="color: rgb(104, 171, 248)">所属年代：</span>
                                        <span v-if="data.bookSubName.length > 60">{{ data.bookSubName.substring(0, 60) }} ...</span>
                                        <span v-else>{{ data.bookSubName }}</span>
                                    </div>
                                    <div>
                                        <span style="color: rgb(104, 171, 248)">出土日期：</span><span>{{ data.publishedDate }}</span>
                                        <span style="margin-left: 50px"></span>
                                        <span style="color: rgb(104, 171, 248)">出土地点：</span>
        ...
        methods: {
                    ...
                    .then(function (result) {
                        ...
                        if (response.suc) {
                            ...
                            if (_this.bookAllData.length == 0) {
                                _this.$message.warning("暂无文物古迹数据");
                                return;
                            }
                            _this.export2Excel();
                        } else {
            ...
            deleteBooks() {
                ...
                if (_this.selectedIds.length == 0) {
                    _this.$message.warning("请选择需要删除的文物古迹");
                    return;
                }
                _this.$confirm(`确定删除该文物古迹数据？`, '提示', {
            ...
            addBook() {
                this.$emit("update:showPage", "relicManage");
            ...
            details(row) {
                this.$emit("update:showPage", "relicDetail");
            ...
            export2Excel() {
                require.ensure([], () => {
                    const tHeader = ['省份', '城市', '区/县', '一级分类', '二级分类', '名称', '级别', '所属年代', '出土地点',
                                       '出土日期', '数量', '简介', '存放位置', '备注', '录入者', '录入时间', '更新时间'];
                    const filterVal = ['province', 'city', 'district', 'classify', 'subClassify', 'bookName', 'author', 'bookSubName', 'publishing',
                                        'publishedDate', 'quantity', 'summary', 'position', 'remarks', 'createUser', 'createTime', 'updateTime'];
                    const list = this.bookAllData;
                    const data = this.formatJson(filterVal, list);
                    export_json_to_excel(tHeader, data, this.classify + '列表');
                })
            },
